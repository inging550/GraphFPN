...
def train_one_epoch(net, optimizer, loss_function, data_loader, device, ema, epoch, args, scaler):
  ...
  with tqdm(data_loader, ncols=50) as tbar:
    for (g, subg_c, subg_h, img, target) in tbar:
      tbar.set_description("Epich:%d" % epoch)
...
def cal_cocomAP(net, dataloader, device, conf_thread, iou_threshold, cls_id):
  ...
  flag = True
  for g, subg_c, subg_h, img, imgid, scale, x_offset, y_offset in tqdm(dataloader):
    img = img.to(device)
    out = net(g, subg_c, subg_h, img)
  ...
  with open('result.json', 'w') as f: # 保存为json文件
    json.dump(results, f)
    coco_true = COCO(annotation_file="/home/featurize/data/instances_val2017.json")
