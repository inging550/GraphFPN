...
from utils.creat_graph import simple_birected, build_edges, heterograph, hetero_subgraph
import dgl
...
class CocoData(Dataset):
  ...
  def __init__(self, label_cachepath, train):
    ...
    self.g = simple_birected(build_edges(heterograph("pixel", 256, 8400)))
  ...
  def __getitem__(self, index):
    ...
    if self.train:
      return self.g, self.subg_c, self.subg_h, img, label
    else:
      return self.g, self.subg_c, self.subg_h, img_path[-16:-4], img, scale, x, y
...
def make_train_data(batch):
  g, subg_c, subg_h, imgs, labels = zip(*batch)
  target = []
  for i,label in enumerate(labels):
    obj_num = label.shape[0]
    img_id = torch.full((obj_num, 1), i)
    target.append(torch.cat((img_id, label),1))  
  batch_g, batch_gc, batch_gh = dgl.batch(g), dgl.batch(subg_c), dgl.batch(subg_h)
  return batch_g, batch_gc, batch_gh, torch.stack(imgs, 0), torch.cat(target, 0)

def make_val_data(batch):
  g, subg_c, subg_h, imgname, imgs, scale, x, y = zip(*batch)
  return dgl.batch(g), dgl.batch(subg_c), dgl.batch(subg_h), torch.stack(imgs, 0), imgname, scale, x, y
